 ...
 *
 */
class LEDMatrix::Implementation
{
private:
...
public:
        ...
        const LedMatrixParameters &params)
    {
        ...
        if (!GPIO_IS_VALID_OUTPUT_GPIO(dataPin))
        {
            ESP_LOGE(
                LOG_TAG,
                "Pin %d is not output-capable in LED matrix",
                dataPin);
            abort();
        }
        ...
        void *arg)
    {
        ...
        {
            LEDMatrix::Implementation *instance =
...
//------------------------------------------------------------------------------
#else
#error There is not a LEDMatrix implementation for your board
#endif
...
LEDMatrix::LEDMatrix(LEDMatrix &&source) : RgbLedController(::std::move(source))
{
    _impl = ::std::move(source._impl);
}

LEDMatrix &LEDMatrix::operator=(LEDMatrix &&source)
{
    _impl = ::std::move(source._impl);
    return static_cast<LEDMatrix &>(
...
void LEDMatrix::show(const PixelVector &pixels)
{
    _impl->show(pixels);
}
...
PixelDriver LEDMatrix::pixelDriver() const noexcept
{
    return _impl->pixelDriver();
}

uint8_t LEDMatrix::brightness()
{
    return _impl->brightness - 1;
}

uint8_t LEDMatrix::brightness(uint8_t value)
{
...
void LEDMatrix::syncWithCPUFrequency()
{
    LEDMatrix::Implementation::syncWithCPUFrequency();
}